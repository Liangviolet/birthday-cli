...
fn main()->Result<()> {
    ...
    match cli.command {
        Command::Add {
            ...
            month,
            year,
        } =>birthday_cli::add(name, day, month, year),
        Command::All {} => {
            let birthdays = birthday_cli::get_all()?;
            output::output(birthdays, today);
            Ok(())
        }
        Command::Next {} => {
            let maybe_birthday = birthday_cli::get_next(today)?;
        ...
        Command::Search {
            ...
            let birthdays = birthday_cli::search(name, year, month, day)?;
            output::output(birthdays, today);
            Ok(())
        }
        Command::Today {} => {
            let today = Utc::now().date_naive();
            let birthdays = birthday_cli::search(
                None,
                Some(today.year()),
                Some(today.month()),
                Some(today.day()),
        ...
        Command::Forget { id } => {
            let maybe_birthday = birthday_cli::forget(id)?;
